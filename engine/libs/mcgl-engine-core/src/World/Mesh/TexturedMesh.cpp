...
TexturedMesh::TexturedMesh( std::vector<VertexCT>&& vertices, std::vector<UNUM32>&& indices ) :
...
void TexturedMesh::generateGLData() {
	...
	glBufferData( GL_ARRAY_BUFFER, vertices_.size() * sizeof( VertexCT ), &vertices_[0], GL_DYNAMIC_DRAW );
	...
	glVertexAttribPointer( 1, 2, GL_FLOAT, GL_FALSE, sizeof( VertexCT ), (void*)offsetof( VertexCT, texCoords_ ) );
